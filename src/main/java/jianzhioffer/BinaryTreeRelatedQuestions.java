 ...
 * 剑指 Offer 28. 对称的二叉树: 请实现一个函数，用来判断一棵二叉树是不是对称的。如果一棵二叉树和它的镜像一样，那么它是对称的。[递归]
 * 剑指 Offer 55 - II. 平衡二叉树: 输入一棵二叉树的根节点，判断该树是不是平衡二叉树。[递归]
 *
 * @author Lynn-zd
...
public class BinaryTreeRelatedQuestions {
     ...
     * @return
     */
    public boolean isSubStructure(TreeNode treeNode1, TreeNode treeNode2) {
        return (treeNode1 != null && treeNode2 != null) &&
                (recur(treeNode1, treeNode2)
                        || isSubStructure(treeNode1.left, treeNode2) ||
                        isSubStructure(treeNode1.right, treeNode2));
    }

    boolean recur(TreeNode treeNode1, TreeNode treeNode2) {
        if (treeNode2 == null) return true;
        if (treeNode1 == null || treeNode1.val != treeNode2.val) return false;
